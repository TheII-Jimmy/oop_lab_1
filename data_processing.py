...
class Table:
    """Create a table of a said data."""

    def __str__(self):
        return self.table_name + ':' + str(self.table)

    def __init__(self, table_name = '', table = []):
        """Initialize Table with name and data"""
        self.table_name = table_name
        self.table = table

    def filter(self, condition):
        """Filter out items that meet the condition"""
        filtered_list = []
        for item in self.table:
            if condition(item):
                filtered_list.append(item)
        return Table(self.table_name + "_filtered", filtered_list)

    def aggregate(self, aggregation_function, aggregation_key):
        """Aggregrate the table in the same key"""
        tmp = []
        for item in self.table:
            try:
    ...
    def join(self, other_table, key):
        ...
        for data in self.table:
            for data2 in other_table.table:
